...
def user_input_features():
    ...
    period_day = st.selectbox('Period Day', ['Afternoon', 'Evening', 'Morning', 'Night'])
    weekday_weekend = st.selectbox('Weekday / Weekend', ['Weekday', 'Weekend'])
    month = st.select_slider("Month", ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Agt', 'Sep', 'Oct', 'Nov', 'Dec'])
...
# Association Rule Mining and Recommendations
...
# Function to return recommended items
...
if type(data) != type("No Result!"):
    ...
    st.success(f"If the customer buys **{item}**, then they also buy **{return_item_df(item)[1]}** together")
else:
